...
We performed a survival analysis using a cox proportional hazards model.  To address differences among treatments, we used nested helmet contrasts asking

1) Does naproxen (at all levels) affect survival relative to the control?

2) Do higher than observed levels of naproxen affect snail survival moreso than observed levels?

3) Does the highest level of naproxen affect snail survival mroeso than the second highest?


```{r Survival Contrasts}
SurvData$TreatmentContrasts <- SurvData$Treatment # create a separate column for contrasts
contrasts(SurvData$TreatmentContrasts) <- contr.helmert(4) # create the nested helmert contrasts
overall_surv <- Surv(SurvData$Week,SurvData$Death) # create the survival object

fit <- coxph(Surv(Week, Death)~TreatmentContrasts, data=SurvData)
fit
```
...
p_1 <- plot_grid(growth_plot,reproduction_plot,surv_plot,ncol=3,labels=c("A","B","C"))
